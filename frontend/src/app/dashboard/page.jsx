    ...
    <main className="ocean-page">
      ...
      <div className="ocean-container">
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <div className="wood-item h-auto flex-col items-start gap-1 py-4"><p className="text-sm font-bold text-[#0d3f63]">Total Projects</p><p className="text-3xl font-black text-[#153852]">{projects.length}</p></div>
          <div className="wood-item h-auto flex-col items-start gap-1 py-4"><p className="text-sm font-bold text-[#0d3f63]">Total Inventory Items</p><p className="text-3xl font-black text-[#153852]">{inventory.length}</p></div>
          <div className="wood-item h-auto flex-col items-start gap-1 py-4"><p className="text-sm font-bold text-[#0d3f63]">Total Experiments</p><p className="text-3xl font-black text-[#153852]">{experiments.length}</p></div>
          <div className="wood-item h-auto flex-col items-start gap-1 py-4"><p className="text-sm font-bold text-[#0d3f63]">Total Requirements</p><p className="text-3xl font-black text-[#153852]">{allRequirements.length}</p></div>
        </div>
              ...
              {paginatedRecentActivity.length ? paginatedRecentActivity.map((activity, index) => (
                <div key={`${activity.text}-${activity.fallback}-${index}`} className="rounded-xl border-2 border-[#0f5f92] bg-[#fff0a5]/95 px-4 py-3">
                  ...
                  <div className="mt-1 flex items-center justify-between gap-2">
                    ...
                    <span className="rounded-full border border-[#0f5f92] bg-[#ffe352] px-2 py-0.5 text-[10px] font-black uppercase tracking-wide text-[#225777]">
                      {activity.level}
                    </span>
                  </div>
                </div>
